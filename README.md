...
- 🌐 [Inference-hook channel](docs/inference-hook.md) screens a whole prompt turn for a model provider that routes prompts to a security server before the model sees them — the same rules, reaching cloud chat surfaces and unmanaged devices where no local hook exists. `prismor inference-hook-server` takes a transcript and answers allow/deny, fail-closed by default
